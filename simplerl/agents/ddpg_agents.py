...
class DDPGAgent(ExplorationNoiseMixin, LocalBufferMixin,
    ...
    def __init__(self, env_fn = None, 
        ...
        assert (hasattr(self.online_network, 'critic_params')), 'TODO'
        ...
        self.register_torch_obj(self.critic_optimizer, 'critic_optimizer')
        ...
        with torch.no_grad():
            if self.n_steps > 1:
                ...
                for i in range(self.n_steps):
                    ...
                    Q = self.target_network.critic_value(next_obs[:, i], 
                                                         next_action)
                    Q_sa_next.append(Q)
                
                Q_sa_next = torch.stack(Q_sa_next, dim = 1)
                
            else:
                next_action = self.target_network.action(next_obs)                              
                Q_sa_next = self.target_network.critic_value(next_obs, 
                                                             next_action)
